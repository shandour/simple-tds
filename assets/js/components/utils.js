...
export const landingPagesWithCountryValues = landingPages =>
  landingPages.map(lp => {
    lp.country = lp.country ? lp.country.value : null;
    return lp;
  });

export const prepareCountriesForReactSelect = landingPages =>
  landingPages.map(lp => {
    lp.country = lp.country
      ? { value: lp.country.code, label: lp.country.name }
      : null;
    return lp;
  });

export const getStatsPerHour = hourlyStats =>
  hourlyStats.map(hS => {
