...
export default ({ history, match: { params } }) => {
  ...
  return (
    <>
      <div>
        <FloatRightEditButton onClick={() => history.push(editLink)}>
          Edit Link
        ...
        <RoundedColDivWithMargin>
          ...
          <StatsDiv>
            {data.linkStats ? (
              <>
                <InfoDiv>
                  Last IP to access the link:{" "}
                  <Link to={`/ip/${data.linkStats.lastIp}`}>
                    {data.linkStats.lastIp}
                  </Link>
                </InfoDiv>
                <InfoDiv>
                  Number of link requests: {data.linkStats.clicks}
                </InfoDiv>
                <InfoDiv>
                  Number of unique request countries:{" "}
                  {data.linkStats.numCountries}
                </InfoDiv>
                <InfoDiv>
                  Number of unique users to have requested the link:{" "}
                  {data.linkStats.numUniqueUsers}
                </InfoDiv>
              </>
            ) : (
            ...
            {data.userStats ? (
              <div>
                ...
                {Object.entries(data.userStats).map(e => (
                  <InfoDiv key={e[0]}>
                    <Link to={`/ip/${e[0]}`}>{e[0]}</Link>
                    -- last access time: {getDate(e[1])}
                  </InfoDiv>
                ))}

                {showChart && <ClicksChart link={params.link} />}
              </div>
            ) : (
